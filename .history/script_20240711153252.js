...
function create() {
    ...
    var directionTextStyle = { fontFamily: 'Marhey', fontSize: '28px', fill: '#000000', align: 'center' };
    var directionText = this.add.text(wodirectionImageodImage.x, directionImage.y, 'الكلمة التالية', directionTextStyle).setOrigin(0.5);
    directionImage.setInteractive();

    // Tween animation for 'pointerover' event
    directionImage.on('pointerover', function() {
        this.tweens.add({
            targets: directionImage,
            y: directionImage.y - 10,  // Move up by 10 pixels
            yoyo: true,
            duration: 200,
            ease: 'Power2'
        });
