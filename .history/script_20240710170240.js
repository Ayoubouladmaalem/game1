...
function create() {
    ...
    var lettersOnCircles = [
        this.add.image(circle1.x, circle1.y, 'spot').setVisible(false), // Initialize with 'spot' hidden
        this.add.image(circle2.x, circle2.y, 'spot').setVisible(false), // Initialize with 'spot' hidden
        this.add.image(circle3.x, circle3.y, 'spot').setVisible(false), // Initialize with 'spot' hidden
        this.add.image(circle4.x, circle4.y, 'spot').setVisible(false), // Initialize with 'spot' hidden
        this.add.image(circle5.x, circle5.y, 'spot').setVisible(false)  // Initialize with 'spot' hidden
    ];

    // Choose a random circle index to place the correct letter
    var correctCircleIndex = Phaser.Math.Between(0, 4);
    lettersOnCircles[correctCircleIndex].setVisible(true); // Make the correct 'spot' visible

    lettersOnCircles.forEach((spotImage, index) => {
        if (index !== correctCircleIndex) {
            spotImage.setInteractive();
            spotImage.on('pointerdown', function() {
                animateImage.call(this, 'sad', spotImage.x, spotImage.y);
            }, this);
        } else {
            spotImage.setInteractive();
            spotImage.on('pointerdown', function() {
                animateImage.call(this, 'happy', spotImage.x, spotImage.y);
