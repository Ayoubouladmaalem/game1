...
function create() {
    ...
    circles.forEach((circle, index) => {
        var circleImage = this.add.image(circle.x, circle.y, 'circle');

        var letterText = this.add.text(circle.x, circle.y, getRandomLetter(letters), textStyle).setOrigin(0.5);

        // Other logic for interactive letters...

        circleImage.setInteractive();
        circleImage.on('pointerover', function() {
            this.tweens.add({
                targets: circleImage,
                y: circleImage.y - 10,
                yoyo: true,
