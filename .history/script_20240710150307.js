...
function create() {
    ...
    var wordText = this.add.text(380, 530, incompleteWord.replace(/./g,"_"), textStyle);
    ...
    lettersOnCircles.forEach((letterText, index) => {
        ...
        letterText.on('pointerdown', function() {
            ...
            if (incompleteWord.includes(letter)) {
                animateImage.call(this, 'happy', letterText.x, letterText.y);
                currentIncompleteWord += letter;
                wordText.setText(incompleteWord.replace(new RegExp("[^" + currentIncompleteWord + "]", "g"), "_"));
