...
function updateCircles(scene) {
    ...
    for (var i = 0; i < circlePositions.length; i++) {
        ...
        // Allow user input for letters on circles
        var defaultLetter = "أ"; // Default or initial letter
        var letterText = scene.add.text(circle.x, circle.y, defaultLetter, { font: "32px Arial", fill: "#000000", align: "center" }).setOrigin(0.5);
        scene.lettersOnCircles.push(letterText);

        // Set interactive behavior for changing letters on circles
        letterText.setInteractive();
        letterText.on('pointerdown', function() {
            var newLetter = prompt("Enter the letter for this circle:", defaultLetter);
            if (newLetter && newLetter.length === 1) {
                letterText.setText(newLetter);
            }
        });
    }
     // Set specific word for `letter.png`
     var staticWord = "وود"; // Replace with your desired static word
     wordText.setText(staticWord);
