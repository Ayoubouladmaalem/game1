...
function preload() {
    ...
    this.load.image('player','assets/ananas.png');
    this.load.image('spot','assets/ananas.png');
...
function create() {
    ...
    this.input.on('pointermove', (pointer) => {
        this.player.x += (pointer.x - this.player.x) * 1;
        this.player.y += (pointer.y - this.player.y) * 1;
    });
...
function update() {
    // Check if the correct letter has been guessed and update the display
    var correctLetterGuessed = currentIncompleteWord.charAt(0) !== beginningLetter;
    
    if (correctLetterGuessed) {
        // Find the circle with the correct letter and replace it with a 'spot'
        var correctCircleIndex = lettersOnCircles.findIndex(text => text.text === beginningLetter);
        animateCircleMove(lettersOnCircles[correctCircleIndex]);

        // Reset current incomplete word and choose a new one for the next round
        currentIncompleteWord = '';
        currentIncompleteWord = incompleteWords[Math.floor(Math.random() * incompleteWords.length)];
        beginningLetter = currentIncompleteWord.charAt(0);

        // Update the displayed word with the new incomplete word
        wordText.setText(getDisplayWordWithHiddenFirstLetter(currentIncompleteWord));

        // Place the new correct letter in a random circle
        correctCircleIndex = Phaser.Math.Between(0, 4);
        lettersOnCircles[correctCircleIndex].setText(beginningLetter);
    }
}

function animateCircleMove(circle) {
    this.tweens.add({
        targets: circle,
        x: circle.x + 10,  // Example movement, adjust as needed
        y: circle.y + 10,
        duration: 500,
        ease: 'Power2'
    });
