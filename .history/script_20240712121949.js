...
function create() {
    ...
    woodText = this.add.text(woodImage.x, woodImage.y, "اختر الحرف المناسب لبداية الكلمة", woodTextStyle).setOrigin(0.5);
    ...
    updateGameElements(this);
}

function updateGameElements(scene) {
    var numCircles = parseInt(document.getElementById("numCircles").value);

    // Update woodText
    var titleInput = document.getElementById("titleInput").value;
    woodText.setText(titleInput);

    // Update letters
    var lettersInput = document.getElementById("lettersInput").value;
    letters = lettersInput.split(',');

    // Update wordText
    var answersInput = document.getElementById("answersInput").value;
    currentIncompleteWord = answersInput;
    beginningLetter = currentIncompleteWord.charAt(0);
    wordText.setText(getDisplayWordWithHiddenFirstLetter(currentIncompleteWord));

    // Update circles
    updateCircles(scene, numCircles);
}

function updateCircles(scene, numCircles) {
    if (!scene.circles) {
        scene.circles = [];
    }
...
function generateCirclePositions(numCircles) {
    ...
    if (numCircles === 1) {
        ...
        positions.push({ x: 400, y: 200 }, { x: 200, y: 200 }, { x: 600, y: 200 }, { x: 400, y: 350 }, { x: 200, y: 350 }, { x: 600, y: 350 });
    } else if (numCircles === 7) {
        positions.push({ x: 400, y: 200 }, { x: 200, y: 200 }, { x: 600, y: 200 }, { x: 400, y: 350 }, { x: 200, y: 350 }, { x: 600, y: 350 }, { x: 300, y: 275 });
    } else if (numCircles === 8) {
        positions.push({ x: 400, y: 200 }, { x: 200, y: 200 }, { x: 600, y: 200 }, { x: 400, y: 350 }, { x: 200, y: 350 }, { x: 600, y: 350 }, { x: 300, y: 275 }, { x: 500, y: 275 });
    }
...
function gameOver(scene) {
    ...
    scene.input.on('pointerdown', function() { window.location.reload(); });
    scene.tweens.add({
        targets: scene.circles,
        alpha: 0,
        duration: 2000,
        ease: 'Power2',
        yoyo: true,
    ...
    var gameOverText = scene.add.text(400, 300, 'حظ أوفر - الملفات قد تنقص', { fontFamily: 'Marhey', fontSize: '48px', fill: '#FDCD2B', align: 'center' }).setOrigin(0.5);
    var gameOverSound = scene.sound.add('gameover');
    gameOverSound.play();
}
