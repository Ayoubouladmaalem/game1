...
function create() {
    ...
    // Initialize player sprite and pointer movement
    this.direction = this.add.sprite(this.sys.game.config.width / 2, this.sys.game.config.height - 50, 'direction').setScale(1);
    this.input.on('pointermove', (pointer) => {
        this.direction.x += (direction.x - this.direction.x) * 0.5;
        this.direction.y += (direction.y - this.direction.y) * 0.5;
    });
}
